...
cp dist/MapExplorer.jar lib/*.map README.txt License.txt lib/MapExplorer.bat lib/mapexplorer tmp/MapExplorer
cd tmp
...
zip -9 tmp/MapExplorer-$version.src.zip `find . -name '*.java' -print` build.xml License.txt \
 lib/*.map README.txt lib/mapexplorer lib/MapExplorer.bat
